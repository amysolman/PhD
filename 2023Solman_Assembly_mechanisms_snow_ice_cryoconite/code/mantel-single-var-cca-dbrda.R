...
###COMBINE DBRDA AND MANTEL TEST PLOTS

#get our titles
title1 <- ggdraw() + draw_label("Mantel Tests", size=20, fontface = "bold")
title2 <- ggdraw() + draw_label("Distance-based Redundancy Analysis", size=20, fontface = "bold")

#final.p2 = pro.p / euk.p / (p1 | p2) / (p3 | p4)

#final.p2 = (pro.p / euk.p) / final.p

final.p2 = title1 / pro.p / euk.p / title2 / (p1 | p2) / (p3 | p4) / leg

top_row = plot_grid(pro.p, euk.p, nrow=2)
bottom_row = plot_grid(p1, p2, p3, p4, nrow=2, align = "hv")
final.p2 = plot_grid(top_row, bottom_row, nrow=2, align = "hv")

#save plot
pdf("../results/rda-mantel-plot.pdf", width=5, height=7)
print(final.p2)
dev.off()
