#clear workspace and load package
rm(list=ls())

library(tidyverse)
library(phyloseq)
library(readr)
library(seqinr)
#BiocManager::install("decontam")
library(decontam)
library(vegan)
library(magrittr)
library(data.table)
library(ape) #appe::Ntip
library(dplyr)
#install.packages("kableExtra")
#library(kableExtra)
library(RColorBrewer) #for plotting colours
library(tidyr) #wide to long format
library(microbiome)
library(cowplot)
library(fantaxtic)

#load data
metadata <- read.csv(file="../data/metadata.csv", sep=",") #Metadata
ps.pro = readRDS("../results/16S-phylo-object.rds")
ps.euk.mm.rm = readRDS("../results/18S-phylo-object-micro-remove.rds")
ps.euk.mm = readRDS("../results/18S-phylo-object-micro-keep.rds")
ps.mm = readRDS("../results/18S-phylo-object-micro-only.rds")

#Dealing with Contaminants

# We can start with the preposition that contamination in our samples is minimal 
#and those ASVs within our negative controls are mainly carry over from our TRUE samples 
#with a small degree of contamination from sample processing reagents/equipment. 
# We can explore this by looking at the presence of bands in gels of our PCR products. 
#After amplifying my samples/blanks I had no visible bands in the blanks. Tapestation of 
#blanks after indexing PCR showed some contamination - this is why these samples were sequenced. 


#Methods

# 1) Do nothing. If your gels are clear of bands/Qubit doesn't register DNA concentration then the impact of contamination is likely negligible (https://www.tandfonline.com/doi/full/10.1657/AAAR0015-062).
# 2) Remove all ASVs that are found in blanks from true samples.
# 3) Look at taxonomy of ASVs in blanks and remove human-associated taxa only.
# 4) Remove samples with amplification levels below controls (https://ami-journals.onlinelibrary.wiley.com/doi/full/10.1111/1462-2920.14366?casa_token=NS2g2Mg8PaUAAAAA%3A4gDRC6RPOXMyB0Wj16e2ynfggh0Coz1-oKN0KgFuJl-FFDKUtkKRe3b8UIS2ugZeCFAdxDL_ZTisgMg)
# 5) USE PREVELANCE METHODS
# 5.1) Use decontam to identify contaminants and remove them.
# I have done this using the prevalence based method. In this method the presence/absence of each ASV in the TRUE samples is compared to the presence/abundance in negative controls to identify contaminants. Those ASVs with greater prevalence in control samples than TRUE samples are considered contaminants with a probability threshold of p < 0.1. I think this means non-contaminants are those with with prevalence in TRUE samples 10x higher than in negative control samples. For example, prevalence of 1 in negative control samples and 20 in TRUE samples would be a non-contaminant. Prevalence of 3 in negative control samples and 20 in TRUE samples would be a contaminant.
# 5.2) For each ASV in the negative controls, calculate mean sum of reads in negative controls and divide by the mean sum of reads across control and TRUE samples. Exclude ASVs with >5% abundance in negative controls. (https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/1365-2745.13344) For example:
# 50/500 = 0.1 (=10%) so this ASV would be excluded.
# 10/500 = 0.02 (=2%) so this ASV would be retained.
# 5.3) Find ASVs in blanks that represent more than 0.1% of reads in all blanks and remove from true samples. Remove ASVs from TRUE samples with less reads than the total number of reads in that sample that come from contaminant ASVs (https://journals.asm.org/doi/full/10.1128/AEM.01253-17).
# 5.4) Find ASVs in blanks that represent more than 0.05% of all reads in blanks and remove from true samples (https://tc.copernicus.org/articles/12/3653/2018/tc-12-3653-2018.pdf).
# 5.5) Remove ASVs that make up >=1% of sequences in negative controls AND >=1% of sequences in TRUE samples (https://zslpublications.onlinelibrary.wiley.com/doi/full/10.1111/jzo.12832?casa_token=XFXCN_1ZenIAAAAA%3ApmvIns4b3wVSw08f961saH_sZ7nZt4Ggkj1ZA79cQZBejcTp-6zRo_yMtZIhdSfcslUhTM512HjYRu8).
# 
# My Method
# 
# 1. Compare library sizes of TRUE and control samples. Control samples should have low read depth (see controls-lib-size.R)
# 2. Make pofile bar plots to get a general idea of the taxa in our controls (see controls-profiles.R)
# 3. Find ASVs that are in blanks. Of those that show high read abundance in TRUE samples (>0.1% total reads) 
# check that they are expected in our environment and not human associated. 
#If they are expected then we will assume these are carry over from TRUE samples. 
#If they are human associated they will be manually removed from the dataset (see controls-high-abundance.R).
# 4. For each remaining ASV in the negative controls, calculate mean sum of reads in negative controls and divide by the mean sum of reads across control and TRUE samples. 
#Exclude ASVs with >5% abundance in negative controls (see controls-remove.R).
# 5. Compare datasets before and after removing ASVs in step 3 using NMDS plots and ANOSIM (see controls-NMDS.R).


# ps = ps.pro
# cont = c("Blank.Snow1", "Blank.Snow2")
# sam = data.frame(sample_data(subset_samples(ps.pro, Habitat == "Snow")))$SampleID

high_abundant_contaminants <- function(ps, cont, sam){
  
  #subset out phyloseq objects by habitat and control samples
  ps.hab <- prune_samples(sam, ps)
  ps.cont = subset_samples(ps, SampleID %in% cont)
  
  #remove zero abundant ASVs
  ps.hab.rm = prune_taxa(taxa_sums(ps.hab) > 0, ps.hab)
  ps.cont.rm = prune_taxa(taxa_sums(ps.cont) > 0, ps.cont)
  
  #find total relative abundance of each taxa in our TRUE samples
  x = data.frame(rowSums(data.frame(otu_table(ps.hab.rm)))/sum(rowSums(data.frame(otu_table(ps.hab.rm)))))
  names(x) = c("Abundance")
  sum(x$Abundance) ==1 #should equal true
  x$id = rownames(x)
  
  #only keep those with high relative abundance in TRUE samples (>0.1%)
  x = x[x$Abundance > 0.001,]
  
  #subset to those in our control samples
  keep = x[rownames(x) %in% taxa_names(ps.cont.rm),]
  
  #get taxonomy
  y = data.frame(tax_table(ps.hab))
  
  #subset ASVs by those found in controls AND true samples
  y.trim = y[rownames(y) %in% keep$id,]
  
  #make id column
  y.trim$id = rownames(y.trim)
  
  #join taxonomic info and abundance data
  y.fin = left_join(y.trim, keep, by="id")
  
  #multiply abundance by 100 and round to get %
  y.fin$Abundance = round(y.fin$Abundance*100, 2)
  
  #remove last row
  #y.fin = y.fin[1:ncol(y.fin)-1]
  
  return(y.fin)
}

#Look for high abundance contaminants

#Prokaryote
cont=c("Blank.Snow1", "Blank.Snow2")
pro.snow.c = high_abundant_contaminants(ps.pro, cont=cont, sam=data.frame(sample_data(subset_samples(ps.pro, Habitat == "Snow")))$SampleID)
cont=c("Blank.Ice1")
pro.sp.ice.c = high_abundant_contaminants(ps.pro, cont=cont, sam=data.frame(sample_data(subset_samples(ps.pro, Habitat == "Spring Ice")))$SampleID)
cont=c("Blank.E", "Blank.Ice2")
pro.sum.ice.c = high_abundant_contaminants(ps.pro, cont=cont, sam=data.frame(sample_data(subset_samples(ps.pro, Habitat == "Summer Ice")))$SampleID)
cont = c("Blank.NC1", "Blank.NC2", "Blank.NC3", "Blank.NC4", "Blank.NC5", "Blank.Cryo")
pro.cryo.c = high_abundant_contaminants(ps.pro, cont=cont, sam=data.frame(sample_data(subset_samples(ps.pro, Habitat == "Cryoconite")))$SampleID)
cont=c("Blank.1", "Blank.2", "Blank.3", "Blank.A", "Blank.B", "Blank.C", "Blank.D")
pro.snow.sp.ice.c = high_abundant_contaminants(ps.pro, cont=cont, sam=data.frame(sample_data(subset_samples(ps.pro, Habitat %in% c("Snow", "Spring Ice"))))$SampleID)
cont=c("Blank.4", "Blank.5", "Blank.6")
pro.cryo.sum.ice.c = high_abundant_contaminants(ps.pro, cont=cont, sam=data.frame(sample_data(subset_samples(ps.pro, Habitat %in% c("Cryoconite", "Summer Ice"))))$SampleID)
cont=c("Blank.PCR.NC")
pro.a.c = high_abundant_contaminants(ps.pro, cont=cont, sam=data.frame(sample_data(subset_samples(ps.pro, Habitat %in% c("Snow", "Spring Ice", "Cryoconite", "Summer Ice"))))$SampleID)

#Eukaryote 
cont=c("Blank.Snow1", "Blank.Snow2")
euk.snow.c = high_abundant_contaminants(ps.euk.mm.rm, cont=cont, sam=data.frame(sample_data(subset_samples(ps.euk.mm.rm, Habitat == "Snow")))$SampleID)
cont=c("Blank.Ice1")
euk.sp.ice.c = high_abundant_contaminants(ps.euk.mm.rm, cont=cont, data.frame(sample_data(subset_samples(ps.euk.mm.rm, Habitat == "Spring Ice")))$SampleID)
cont=c("Blank.E", "Blank.Ice2")
euk.sum.ice.c = high_abundant_contaminants(ps.euk.mm.rm, cont=cont, data.frame(sample_data(subset_samples(ps.euk.mm.rm, Habitat == "Summer Ice")))$SampleID)
cont = c("Blank.NC1", "Blank.NC2", "Blank.NC3", "Blank.NC4", "Blank.NC5", "Blank.Cryo")
euk.cryo.c = high_abundant_contaminants(ps.euk.mm.rm, cont=cont, data.frame(sample_data(subset_samples(ps.euk.mm.rm, Habitat == "Cryoconite")))$SampleID)
cont=c("Blank.1", "Blank.2", "Blank.3", "Blank.A", "Blank.B", "Blank.C", "Blank.D")
euk.snow.sp.ice.c = high_abundant_contaminants(ps.euk.mm.rm, cont=cont, data.frame(sample_data(subset_samples(ps.euk.mm.rm, Habitat %in% c("Snow", "Spring Ice"))))$SampleID)
cont=c("Blank.4", "Blank.5", "Blank.6")
euk.cryo.sum.ice.c = high_abundant_contaminants(ps.euk.mm.rm, cont=cont, data.frame(sample_data(subset_samples(ps.euk.mm.rm, Habitat %in% c("Cryoconite", "Summer Ice"))))$SampleID)
cont=c("Blank.PCR.NC")
euk.a.c = high_abundant_contaminants(ps.euk.mm.rm, cont=cont, data.frame(sample_data(subset_samples(ps.euk.mm.rm, Habitat %in% c("Snow", "Spring Ice", "Cryoconite", "Summer Ice"))))$SampleID)

#Save results
export_res = rbind(pro.snow.c, pro.sp.ice.c, pro.sum.ice.c, pro.cryo.c, pro.snow.sp.ice.c, pro.cryo.sum.ice.c, pro.a.c,
                   euk.snow.c, euk.sp.ice.c, euk.sum.ice.c, euk.cryo.c, euk.snow.sp.ice.c, euk.cryo.sum.ice.c, euk.a.c)
export_res = cbind(Group=c(rep("Prokaryote Snow", nrow(pro.snow.c)), 
                           rep("Prokaryote Spring Ice", nrow(pro.sp.ice.c)),
                           rep("Prokaryote Summer Ice", nrow(pro.sum.ice.c)),
                           rep("Prokaryote Cryoconite", nrow(pro.cryo.c)),
                           rep("Prokaryote Snow and Spring Ice", nrow(pro.snow.sp.ice.c)),
                           rep("Prokaryote Cryoconite and Summer Ice", nrow(pro.cryo.sum.ice.c)),
                           rep("All", nrow(pro.a.c)),
                           rep("Eukaryote Snow", nrow(euk.snow.c)), 
                           rep("Eukaryote Spring Ice", nrow(euk.sp.ice.c)),
                           rep("Eukaryote Summer Ice", nrow(euk.sum.ice.c)),
                           rep("Eukaryote Cryoconite", nrow(euk.cryo.c)),
                           rep("Eukaryote Snow and Spring Ice", nrow(euk.snow.sp.ice.c)),
                           rep("Eukaryote Cryoconite and Summer Ice", nrow(euk.cryo.sum.ice.c)),
                           rep("All", nrow(euk.a.c))), 
                   export_res)

write.csv(export_res, "../results/high-abundance-contaminants.csv")
