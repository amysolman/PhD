...
p1 = ggplot(data=final.df) +
  ...
  geom_text(data=final.num, aes(label = paste("R^2 == ", round(Rsqr, 3))), x=-4.9, y=0.8, size=10, parse=TRUE) +
  geom_text(data=final.num, aes(label = paste("italic(m) ==", round(m, 4))), x=-4.9, y=0.68, size=10, parse=TRUE) +
...
p3 = ggplot(df.2.plot,                         
  ...
  geom_text(
    aes(label = paste(round(Perc, 1), "%")),
    position = position_fill(0.5), size=8
  )+
