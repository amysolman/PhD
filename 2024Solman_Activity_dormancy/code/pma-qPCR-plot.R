...
stats::wilcox.test(Gene.Copies.Per.mL.mg ~ Treatment, data = df[df$Season == "Summer",], paired = TRUE, alternative="less")

mean(df[df$Season == "Summer" & df$Treatment == "iDNA",]$Gene.Copies.Per.mL.mg)
mean(df[df$Season == "Summer" & df$Treatment == "tDNA",]$Gene.Copies.Per.mL.mg)
