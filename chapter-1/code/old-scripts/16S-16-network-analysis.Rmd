...
#subset data by pole
antarctic <- subset_samples(phylo, Pole=="Antarctic")
...
#antarctic.filter <- subset_taxa(antarctic, Phylum != "NA")
```

Step Three: Normalize data
```{r}
#draw rarefaction curves
rarecurve(t(otu_table(antarctic)), step=50, cex=0.5)

#what is the minimum number of reads
min(sample_sums(antarctic)) #we have some samples with no reads!

#remove samples with less than 10000 reads (we need at least 10000 reads per sample)
antarctic.prune = prune_samples(sample_sums(antarctic)>=26800, antarctic)

#if we re-plot our curve we can see the smallest sample was removed
rarecurve(t(otu_table(antarctic.prune)), step=50, cex=0.5)

#what is the minimum number of sequences now?
min(sample_sums(antarctic.prune)) #31843

#rarefy data to minimum num sequences
ps.rarefied = rarefy_even_depth(antarctic.prune, rngseed=1, sample.size=min(sample_sums(antarctic.prune)), replace=F)
```
...
Step Five: Filter ASVs to simplify network analysis
```{r}
#NOTE: After rarefying to 10434 reads only one ASV was present in over 10% of samples. Because of this I have increase the minimum number of reads per samples - this means with lose samples but we will keep more ASVs present in more than one sample and this is needed for network analysis. 
...
length(cor_pp[cor_pp > 0 & cor_pp < 0.01]) #884 - only a small number of our correlations are significant
...
length(cor_r[cor_r < -0.6]) #there are 0 negative correlation coefficients
length(cor_r[cor_r > 0.6]) #there 19223 positive correlation coefficients
...
length(cor[cor > 0.6]) #884 positive correlations
...
#export graph
write.graph(cor_g,"results/antarctic_network.gml", format="gml")
```
...
gorder(cor_g) #total = 51
gorder(g_ra) #rare = 13
gorder(g_abd) #abundant = 15
gorder(g_int) #intermediate = 23

#number of edges
gsize(cor_g) #total = 442
gsize(g_ra) #rare = 30
gsize(g_abd) #abundant = 50
gsize(g_int) #intermediate = 60
...
# 50 abundant - abundant correlations
# 79+52 = 131 abundant - intermediate correlations
# 62+19 = 81 abundant - rare correlations
# 60 intermediate - intermediate correlations
# 59+31=90 intermediate - rare correlations
# 30 rare - rare correlations
```
...
TukeyHSD(res.aov) #none of the categories are significantly different
...
plot(res.aov, 2) #points do not follow the line
...
kruskal.test(betweenness ~ category, data = df) #p > 0.05 so categories are NOT significantly different
kruskal.test(closeness ~ category, data = df) #p > 0.05 so categories are NOT significantly different
kruskal.test(eigenvector ~ category, data = df) #p > 0.05 so categories are NOT significantly different

#We don't need todo the below analysis because there was no significant differences
#Wilcox test to look for differences between categories non-parametric
...
# 
# #Betweenness
# wilcox.test(rare_data$betweenness, abundant_data$betweenness, alternative = "two.sided") 
# wilcox.test(rare_data$betweenness, intermediate_data$betweenness, alternative = "two.sided") 
# wilcox.test(abundant_data$betweenness, intermediate_data$betweenness, alternative = "two.sided") 
# 
# #Closeness
# wilcox.test(rare_data$closeness, abundant_data$closeness, alternative = "two.sided")
# wilcox.test(rare_data$closeness, intermediate_data$closeness, alternative = "two.sided") 
# wilcox.test(abundant_data$closeness, intermediate_data$closeness, alternative = "two.sided") 
# 
# #Eigenvector
...
taxonomy <- data.frame(tax_table(antarctic))
...
fit_power_law(cor_g) #does appear to be scale-free but not many data points!
