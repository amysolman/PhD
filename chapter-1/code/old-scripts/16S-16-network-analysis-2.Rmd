...
network_data_prep <- function(phylo1, perc){
  ...
  rare1 <- asv_tab1[,colSums(asv_tab1)/nrow(asv_tab1)< 0.00001] 
  abundant1 <- asv_tab1[,colSums(asv_tab1)/nrow(asv_tab1)>0.0005] 
  intermediate1 <- asv_tab1[,colSums(asv_tab1)/nrow(asv_tab1)>=0.00001 & colSums(asv_tab1)/nrow(asv_tab1)<=0.0005] 
  ...
  # abundance_class_df <- data.frame(ID = c(names(rare1), names(abundant1), names(intermediate1), names(rare2), names(abundant2), names(intermediate2)), Class=c(rep("Rare", ncol(rare1)), rep("Abundant", ncol(abundant1)), rep("Intermediate", ncol(intermediate1)), rep("Rare", ncol(rare2)), rep("Abundant", ncol(abundant2)), rep("Intermediate", ncol(intermediate2))), Kingdom=c(rep("Prokaryote", ncol(rare1)+ncol(abundant1)+ncol(intermediate1)), rep("Eukaryote", ncol(rare2)+ncol(abundant2)+ncol(intermediate2))))  
    ...
    abundance_class_df <- data.frame(ID = c(names(rare1), names(abundant1), names(intermediate1)), Class=c(rep("Rare", ncol(rare1)), rep("Abundant", ncol(abundant1)), rep("Intermediate", ncol(intermediate1))))  
  
  #filter ASVs to those present in at least % of samples
   pro_filter <- filter_me(asv_tab1, perc)
   ...
   newList <- list("class" = abundance_class_df, "asvs" = pro_filter)
...
get_my_network <- function(phylo1, perc){
  
  #get our asv table
  output <- network_data_prep(phylo1, perc)
  asv_tab <- output$asvs
...
node_level_properties <- function(phylo1, phylo2){
