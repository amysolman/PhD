...
```{r}
abundance_frequences_cutoffs <- function(phylo, group, meta){
...
p <- ggplot(df, aes(x=x, y=freq)) + 
  ...
  ylab("Frequency")+
  ggtitle(paste0("Frequency of Mean Relative Abundance of ", group, " ", meta, " ASVs"))+
  theme_bw()

    pdf(paste0("../results/", group, "/graphs/abundance-class/", group, "-", meta, "-abundance-frequency-cutoffs.pdf"))
    print(p)
    dev.off()
...
```{r}
#Prokaryotic Arctic
pro_arc <- subset_samples(ps_pro, Pole == "Arctic")
pro_arc_my_data <- abundance_frequences_cutoffs(pro_arc, "16S", "Arctic")
pro_arc_all_data <- findabundance(pro_arc_my_data)

#Prokaryotic Antarctic
pro_ant <- subset_samples(ps_pro, Pole == "Antarctic")
pro_ant_my_data <- abundance_frequences_cutoffs(pro_ant, "16S", "Antarctic")
pro_ant_all_data <- findabundance(pro_ant_my_data)

#Eukaryotic Arctic
euk_arc <- subset_samples(ps_euk, Pole == "Arctic")
euk_arc_my_data <- abundance_frequences_cutoffs(euk_arc, "18S", "Arctic")
euk_arc_all_data <- findabundance(euk_arc_my_data)

#Eukaryotic Antarctic
euk_ant <- subset_samples(ps_euk, Pole == "Antarctic")
euk_ant_my_data <- abundance_frequences_cutoffs(euk_ant, "18S", "Antarctic")
euk_ant_all_data <- findabundance(euk_ant_my_data)
```

Classify taxa: Region
```{r}
#Prokaryotic McMurdo Dry Valleys
pro_mc <- subset_samples(ps_pro, Region == "McMurdo_Dry_Valleys")
pro_mc_my_data <- abundance_frequences_cutoffs(pro_mc, "16S", "McMurdo_Dry_Valleys")
pro_mc_all_data <- findabundance(pro_mc_my_data)

#Prokaryotic Queen Maud Land
pro_qm <- subset_samples(ps_pro, Region == "Queen_Maud_Land")
pro_qm_my_data <- abundance_frequences_cutoffs(pro_qm, "16S", "Queen_Maud_Land")
pro_qm_all_data <- findabundance(pro_qm_my_data)

#Prokaryotic Svalbard
pro_sv <- subset_samples(ps_pro, Region == "Svalbard")
pro_sv_my_data <- abundance_frequences_cutoffs(pro_sv, "16S", "Svalbard")
pro_sv_all_data <- findabundance(pro_sv_my_data)

#Prokaryotic Sweden
pro_sw <- subset_samples(ps_pro, Region == "Sweden")
pro_sw_my_data <- abundance_frequences_cutoffs(pro_sw, "16S", "Sweden")
pro_sw_all_data <- findabundance(pro_sw_my_data)

#Prokaryotic Greenland
pro_gr <- subset_samples(ps_pro, Region == "Greenland")
pro_gr_my_data <- abundance_frequences_cutoffs(pro_gr, "16S", "Greenland")
pro_gr_all_data <- findabundance(pro_gr_my_data)

#Eukaryotic McMurdo Dry Valleys
euk_mc <- subset_samples(ps_euk, Region == "McMurdo_Dry_Valleys")
euk_mc_my_data <- abundance_frequences_cutoffs(euk_mc, "18S", "McMurdo_Dry_Valleys")
euk_mc_all_data <- findabundance(euk_mc_my_data)

#Eukaryotic Queen Maud Land - No 18S data for these samples
# euk_qm <- subset_samples(ps_euk, Region == "Queen_Maud_Land")
# euk_qm_my_data <- abundance_frequences_cutoffs(euk_qm, "18S", "Queen_Maud_Land")
# euk_qm_all_data <- findabundance(euk_qm_my_data)

#Eukaryotic Svalbard
euk_sv <- subset_samples(ps_euk, Region == "Svalbard")
euk_sv_my_data <- abundance_frequences_cutoffs(euk_sv, "18S", "Svalbard")
euk_sv_all_data <- findabundance(euk_sv_my_data)

#Eukaryotic Sweden
euk_sw <- subset_samples(ps_euk, Region == "Sweden")
euk_sw_my_data <- abundance_frequences_cutoffs(euk_sw, "18S", "Sweden")
euk_sw_all_data <- findabundance(euk_sw_my_data)

#Eukaryotic Greenland
euk_gr <- subset_samples(ps_euk, Region == "Greenland")
euk_gr_my_data <- abundance_frequences_cutoffs(euk_gr, "18S", "Greenland")
euk_gr_all_data <- findabundance(euk_gr_my_data)

```


Subset phyloseq objects by abundance cutoffs: Pole
```{r}
#get names of rare ASVs from Arctic region under each scenario
pro_arc_rare_one <- rownames(pro_arc_all_data[pro_arc_all_data$Low_Low == "Rare",])
pro_arc_rare_two <- rownames(pro_arc_all_data[pro_arc_all_data$Mid_Low == "Rare",])
pro_arc_rare_three <- rownames(pro_arc_all_data[pro_arc_all_data$High_Low == "Rare",])
pro_arc_rare_four <- rownames(pro_arc_all_data[pro_arc_all_data$Low_Low == "Rare",])
