...
#TOTAL DATASET
#ps <- readRDS("../../results/18S/phylo-objects/18S-phyloseq-object-rarefied-decontam.rds")
# ps <- readRDS("data-output/18S-phyloseq-object-rarefied.rds")
#ps2 <- readRDS("../16S/data-output/16S-phyloseq-object-rarefied-decontam.rds")

# #Total Abundant
#ps.int <- readRDS("data-output/18S-total-abundant.rds")
# #Total Intermediate
ps.int <- readRDS("data-output/18S-total-intermediate.rds")
# #Total Rare
...
#new.ps <- rarefy_even_depth(ps, sample.size = 2)

rel_abund <- transform_sample_counts(ps.int, function(x) x/sum(x))
# rel_abund2 <- transform_sample_counts(ps2, function(x) x/sum(x))

# #get sample data
# samp_data1 <- data.frame(sample_data(rel_abund1))
# samp_data2 <- data.frame(sample_data(rel_abund2))
# 
# drops <- c("Bardcode_53_16S")
# samp_data1 <- samp_data1[,!(names(samp_data1) %in% drops)]
# #samp_data1 <- subset(samp_data1, select=-c(Barcode_53_16S))
...
meta_df <- function(phylo){
  ...
  keeps <- c("Conductivity", "pH",
             "Cl_merge", "SO4_merge", "Na_merge", "K_merge", "Mg_merge","Ca_merge")
  ...
  #change sample names 
  names(samp_data) <- c("Conductivity", "pH", "Cl", "SO4", "Na", "K", "Mg","Ca")
  
  #remove columns with more than 50% missing variables
  samp_data_trim <- samp_data[ lapply( samp_data, function(x) sum(is.na(x)) / length(x) ) < 0.7 ]
  
  #Only keep complete cases
  samp_data_trim_complete <- samp_data_trim[complete.cases(samp_data_trim),]
  
  #make sure data are numeric
  samp_data_trim_complete[1:8] <- data.frame(lapply(samp_data_trim_complete[1:8],as.numeric))
...
#Conductivity
Cond <- asv.niche.df$Conductivity #get niche preferences
...
#Save crlg table
write.table(crlg, "results/18S-intermediate-crlg.txt")
...
p <- ggplot(data=crlg, aes(x=class.index, y=Mantel.cor)) +
  ...
  ggtitle("Intermediate Community Mantel Correlogram")

#save pdf
pdf("results/18S-intermediate-mantel-correlogram.pdf")
print(p)
dev.off()
...
ps.norm = transform_sample_counts(ps.int, function(x) x/sum(x))
...
#full.bNTI.df = Phylo_turnover(ps.norm, 10, 10)
full.bNTI.df = Phylo_turnover(ps.norm, 1000, 10)
#Save bNTI table
write.table(full.bNTI.df, "results/18S-intermediate-full_bNTI.txt")
...
#get RC Bray results
#RCb.df = Calc_RCbray(ps.norm, 2, 2)
RCb.df = Calc_RCbray(ps.norm, 999, 20)
#Save RCb.df table
write.table(RCb.df, "results/18S-intermediate-RCb.df.txt")

#Plot results
x.1 = data.frame(sample_data(ps.int)) %>%
...
x.2 = data.frame(sample_data(ps.int)) %>%
  select(SampleID, Glacier) %>%
...
#Save within.bNTI.df
write.table(within.bNTI.df, "results/18S-intermediate-within.bNTI.df.txt")
...
eco.bNTI.plot = ggplot(within.bNTI.df, aes(x=Glacier, y=bNTI)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(size=14))+
  ylim(-2, 10)+
  ggtitle("Intermediate Taxa BNTI Results")


#save pdf
pdf("results/18S-intermediate-eco.bNTI.plot.pdf")
print(eco.bNTI.plot)
dev.off()
...
write.table(eco.turnover.df, "results/18S-intermediate-eco.turnover.df.txt")
...
#Save final_data
write.table(final_data, "results/18S-intermediate-final_data.txt")
...
eco.turnover.plot = ggplot(final_data, aes(x=Glacier, y=perc, fill=process)) +
  ...
  ggtitle("Intermediate Taxa RC Bray Results")


#save pdf
pdf("results/18S-intermediate-eco.turnover.plot.pdf")
...
#save pdf
pdf("results/18S-intermediate-eco.plot.pdf")
print(eco.plot)
dev.off() 
