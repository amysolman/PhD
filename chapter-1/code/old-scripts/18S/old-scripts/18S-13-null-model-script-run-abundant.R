...
#abundantfy to small sample depth so I can run this code on my computer
#new.ps <- abundantfy_even_depth(ps.abundant, sample.size = 2)
...
# abundantfy to an even depth
set.seed(72)  # setting seed for reproducibility
# bulk.physeq.abundant = abundantfy_even_depth(bulk.physeq)
...
eco.bNTI.plot = ggplot(within.bNTI.df, aes(x=Glacier, y=bNTI)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(size=14))+
  ylim(-2, 10)+
  ggtitle("Abundant Community BNTI Results")
...
eco.turnover.plot = ggplot(final_data, aes(x=Glacier, y=perc, fill=process)) +
  ...
  ggtitle("Abundant Community RC Bray Results")
