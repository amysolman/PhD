...
# #Total Rare
ps.rare <- readRDS("data-output/18S-total-rare.rds")
...
rel_abund <- transform_sample_counts(ps.rare, function(x) x/sum(x))
...
#Save crlg table
write.table(crlg, "results/18S-total-crlg.txt")
...
p <- ggplot(data=crlg, aes(x=class.index, y=Mantel.cor)) +
  ...
  ggtitle("Total Community Mantel Correlogram")

#save pdf
pdf("results/18S-total-mantel-correlogram.pdf")
print(p)
dev.off()
...
ps.norm = transform_sample_counts(ps.rare, function(x) x/sum(x))
...
#Save bNTI table
write.table(full.bNTI.df, "results/18S-total-full_bNTI.txt")
...
#Save RCb.df table
write.table(RCb.df, "results/18S-rare-RCb.df.txt")

#Plot results
x.1 = data.frame(sample_data(ps.rare)) %>%
...
x.2 = data.frame(sample_data(ps.rare)) %>%
  select(SampleID, Glacier) %>%
...
#Save within.bNTI.df
write.table(within.bNTI.df, "results/18S-rare-within.bNTI.df.txt")
...
eco.bNTI.plot = ggplot(within.bNTI.df, aes(x=Glacier, y=bNTI)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(size=14))+
  ylim(-2, 10)+
  ggtitle("Rare Taxa BNTI Results")


#save pdf
pdf("results/18S-rare-eco.bNTI.plot.pdf")
print(eco.bNTI.plot)
dev.off()
...
write.table(eco.turnover.df, "results/18S-rare-eco.turnover.df.txt")
...
#Save final_data
write.table(final_data, "results/18S-rare-final_data.txt")
...
eco.turnover.plot = ggplot(final_data, aes(x=Glacier, y=perc, fill=process)) +
  ...
  ggtitle("Rare Taxa RC Bray Results")


#save pdf
pdf("results/18S-rare-eco.turnover.plot.pdf")
...
#save pdf
pdf("results/18S-rare-eco.plot.pdf")
print(eco.plot)
dev.off() 
