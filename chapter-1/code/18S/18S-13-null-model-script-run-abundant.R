...
# #Total Abundant
ps.abun <- readRDS("data-output/18S-total-abundant.rds")
# #Total Intermediate
...
rel_abund <- transform_sample_counts(ps.abun, function(x) x/sum(x))
...
#Save crlg table
write.table(crlg, "results/18S-abundant-crlg.txt")
...
p <- ggplot(data=crlg, aes(x=class.index, y=Mantel.cor)) +
  ...
  ggtitle("Abundant Community Mantel Correlogram")

#save pdf
pdf("results/18S-abundant-mantel-correlogram.pdf")
print(p)
dev.off()
...
ps.norm = transform_sample_counts(ps.abun, function(x) x/sum(x))
...
#Save bNTI table
write.table(full.bNTI.df, "results/18S-abundant-full_bNTI.txt")
...
#Save RCb.df table
write.table(RCb.df, "results/18S-abundant-RCb.df.txt")

#Plot results
x.1 = data.frame(sample_data(ps.abun)) %>%
...
x.2 = data.frame(sample_data(ps.abun)) %>%
  select(SampleID, Glacier) %>%
...
#Save within.bNTI.df
write.table(within.bNTI.df, "results/18S-abundant-within.bNTI.df.txt")
...
eco.bNTI.plot = ggplot(within.bNTI.df, aes(x=Glacier, y=bNTI)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(size=14))+
  ylim(-2, 10)+
  ggtitle("Abundant Taxa BNTI Results")


#save pdf
pdf("results/18S-abundant-eco.bNTI.plot.pdf")
print(eco.bNTI.plot)
dev.off()
...
write.table(eco.turnover.df, "results/18S-abundant-eco.turnover.df.txt")
...
#Save final_data
write.table(final_data, "results/18S-abundant-final_data.txt")
...
#save pdf
pdf("results/18S-abundant-eco.turnover.plot.pdf")
...
#save pdf
pdf("results/18S-abundant-eco.plot.pdf")
print(eco.plot)
dev.off() 
