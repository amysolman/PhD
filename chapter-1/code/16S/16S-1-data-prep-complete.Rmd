...
#Step Seven: remove abiguous annotations
```{r}
ps.ambig <- subset_taxa(ps.merge2, !is.na(Kingdom) & !Kingdom %in% c("Unassigned"))
ps.ambig

ps.ambig_clean <- subset_taxa(ps.merge2_clean, !is.na(Kingdom) & !Kingdom %in% c("Unassigned"))
ps.ambig_clean
```
...
ps.prune_clean <- prune_samples(sample_sums(ps.ambig_clean) >= 12000, ps.ambig_clean) 
ps.prune_clean
```
...
ps.rar_clean = rarefy_even_depth(ps.prune_clean, rngseed=1, sample.size=0.95*min(sample_sums(ps.prune_clean)), replace=F)
ps.rar_clean
