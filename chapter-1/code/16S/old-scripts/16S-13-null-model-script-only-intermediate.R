...
ps.glaciers.merged.I <- rarefy_even_depth(ps.glaciers.merged.I, sample.size = 2)

rel_abund <- transform_sample_counts(ps.glaciers.merged.I, function(x) x/sum(x))


# 2 Edit metadata and fill missing data with median values

metadata <- data.frame(sample_data(ps.glaciers.merged.I))
...
#Save crlg table
write.table(crlg, "results/intermediate-crlg.txt")
...
#save pdf
pdf("results/intermediate-mantel-correlogram.pdf")
print(p)
dev.off()
...
#Save bNTI table
write.table(full.bNTI.df, "results/intermediate-full-bNTI.txt")
...
#Save RCb.df table
write.table(RCb.df, "results/intermediate-RCb-df.txt")
...
#Save within.bNTI.df
write.table(within.bNTI.df, "results/intermediate-within-bNTI-df.txt")
...
eco.bNTI.plot = ggplot(within.bNTI.df, aes(x=Glacier, y=bNTI)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(size=14))+
  ylim(-2, 10)+
  ggtitle(paste0("Intermediate Taxa \u03B2NTI Results"))


#save pdf
# pdf("results/intermediate-eco-bNTI-plot.pdf")
# print(eco.bNTI.plot)
# dev.off()

ggsave(file="results/intermediate-eco-bNTI-plot.svg", plot=eco.bNTI.plot)
...
write.table(eco.turnover.df, "results/intermediate-eco-turnover-df.txt")
...
#Save final_data
write.table(final_data, "results/intermediate-final-data.txt")
...
#save pdf
# pdf("results/intermediate-eco-turnover-plot.pdf")
# print(eco.turnover.plot)
# dev.off()

ggsave(file="results/intermediate-eco-turnover-plot.svg", plot=eco.turnover.plot)
...
eco.plot = cowplot::plot_grid(eco.bNTI.plot, eco.turnover.plot, 
                              labels=c("A", "B"), ncol=1)
#save pdf
# pdf("results/intermediate-eco-plot.pdf")
# print(eco.plot)
# dev.off()

ggsave(file="results/intermediate-eco-plot.svg", plot=eco.plot)
