...
# ---
#   This script will carry out null model analysis on my communities. Code taken from 
...
library(parallel)
#install.packages('svglite') #for saving image as svg
library(svglite)
...
# ps.rar <- rarefy_even_depth(ps.rar, sample.size = 2)

rel_abund <- transform_sample_counts(ps.rar, function(x) x/sum(x))


# 2 Edit metadata and fill missing data with median values

metadata <- data.frame(sample_data(ps.rar))
...
#Save crlg table
write.table(crlg, "results/total-crlg.txt")
...
#save pdf
pdf("results/total-mantel-correlogram.pdf")
print(p)
dev.off()
...
#ps.rar.norm = transform_sample_counts(ps.rar, function(x) x/sum(x))
...
#Save bNTI table
write.table(full.bNTI.df, "results/total-full-bNTI.txt")
...
#Save RCb.df table
write.table(RCb.df, "results/total-RCb-df.txt")
...
#Save within.bNTI.df
write.table(within.bNTI.df, "results/total-within-bNTI-df.txt")
...
eco.bNTI.plot = ggplot(within.bNTI.df, aes(x=Glacier, y=bNTI)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_text(size=14))+
  ylim(-2, 10)+
  ggtitle(paste0("Total Taxa \u03B2NTI Results"))


#save pdf
# pdf("results/total-eco-bNTI-plot.pdf")
# print(eco.bNTI.plot)
# dev.off()

ggsave(file="results/total-eco-bNTI-plot.svg", plot=eco.bNTI.plot)
...
write.table(eco.turnover.df, "results/total-eco-turnover-df.txt")
...
#Save final_data
write.table(final_data, "results/total-final-data.txt")
...
#save pdf
# pdf("results/total-eco-turnover-plot.pdf")
# print(eco.turnover.plot)
# dev.off()

ggsave(file="results/total-eco-turnover-plot.svg", plot=eco.turnover.plot)
...
eco.plot = cowplot::plot_grid(eco.bNTI.plot, eco.turnover.plot, 
                              labels=c("A", "B"), ncol=1)
#save pdf
# pdf("results/total-eco-plot.pdf")
# print(eco.plot)
# dev.off()

ggsave(file="results/total-eco-plot.svg", plot=eco.plot)
