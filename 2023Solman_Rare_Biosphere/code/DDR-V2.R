...
#for arctic an antarctic full communities separately 
  
  #Calculate Bray-Curtis dissimilarities between sites
  pro.arctic = subset_samples(pro, Pole == "Arctic")
  
  #get out count tables
  x1 = data.frame(otu_table(pro.arctic), check.names=FALSE)
  
  #create empty dataframe with ASVs as rows and sample names as columns
  nams = c(colnames(x1)) #sample names
  row.nams = taxa_names(pro.arctic) #ASV IDs
  final.df <- data.frame(matrix(ncol = length(nams), nrow = length(row.nams)))
  colnames(final.df) = nams
  row.names(final.df) = row.nams
  final.df[is.na(final.df)] <- 0
  ...
  # 4. Calculate geographic distance between sites
  #data = data.frame(sample_data(phylo))
  geo = get_my_long_lat(pro.arctic)
  
  #make sure we've got the same samples in our geo data and com dissimilarity data
  geo = geo[rownames(geo) %in% names(comm.out),]
  
  nrow(geo) == nrow(t(comm.out)) #should be true
  rownames(geo) == rownames(t(comm.out)) #should all be true
  
  geo.dist <- earth.dist(geo, dist=TRUE) #distance in kilometers
  
  #Carry out least-squares linear regression on log-transformed geographic distance and log-transformed community dissimilarity
  ...
  #fit the model  
  pro.arc.fit <- lm(Beta ~ km, data = data)
  #model results
  summary(pro.arc.fit)
  
  
  #Calculate Bray-Curtis dissimilarities between sites
  pro.antarctic = subset_samples(pro, Pole == "Antarctic")
  
  #get out count tables
  x1 = data.frame(otu_table(pro.antarctic), check.names=FALSE)
  
  #create empty dataframe with ASVs as rows and sample names as columns
  nams = c(colnames(x1)) #sample names
  row.nams = taxa_names(pro.antarctic) #ASV IDs
  final.df <- data.frame(matrix(ncol = length(nams), nrow = length(row.nams)))
  colnames(final.df) = nams
  row.names(final.df) = row.nams
  final.df[is.na(final.df)] <- 0
  ...
  # 4. Calculate geographic distance between sites
  #data = data.frame(sample_data(phylo))
  geo = get_my_long_lat(pro.antarctic)
  ...
  #fit the model  
  pro.ant.fit <- lm(Beta ~ km, data = data)
  #model results
  summary(pro.ant.fit)
  ...
  #Calculate Bray-Curtis dissimilarities between sites
  euk.arctic = subset_samples(euk, Pole == "Arctic")
  
  #get out count tables
  x1 = data.frame(otu_table(euk.arctic), check.names=FALSE)
  
  #create empty dataframe with ASVs as rows and sample names as columns
  nams = c(colnames(x1)) #sample names
  row.nams = taxa_names(euk.arctic) #ASV IDs
  final.df <- data.frame(matrix(ncol = length(nams), nrow = length(row.nams)))
  colnames(final.df) = nams
  row.names(final.df) = row.nams
  final.df[is.na(final.df)] <- 0
  ...
  # 4. Calculate geographic distance between sites
  #data = data.frame(sample_data(phylo))
  geo = get_my_long_lat(euk.arctic)
  ...
  #fit the model  
  euk.arc.fit <- lm(Beta ~ km, data = data)
  #model results
  summary(euk.arc.fit)
  
  
  #Calculate Bray-Curtis dissimilarities between sites
  euk.antarctic = subset_samples(euk, Pole == "Antarctic")
  
  #get out count tables
  x1 = data.frame(otu_table(euk.antarctic), check.names=FALSE)
  
  #create empty dataframe with ASVs as rows and sample names as columns
  nams = c(colnames(x1)) #sample names
  row.nams = taxa_names(euk.antarctic) #ASV IDs
  final.df <- data.frame(matrix(ncol = length(nams), nrow = length(row.nams)))
  colnames(final.df) = nams
  row.names(final.df) = row.nams
  final.df[is.na(final.df)] <- 0
  ...
  # 4. Calculate geographic distance between sites
  #data = data.frame(sample_data(phylo))
  geo = get_my_long_lat(euk.antarctic)
  ...
  #fit the model  
  euk.ant.fit <- lm(Beta ~ km, data = data)
  #model results
  summary(euk.ant.fit)
