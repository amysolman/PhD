...
#Count Table
pro_count_table <- read_tsv(file="../qiime-export/16S/table.tsv", skip=1)
...
#Rest of the data
pro_tax <- read_tsv(file="../qiime-export/16S/taxonomy.tsv")
pro_fasta <- read.fasta(file="../qiime-export/16S/dna-sequences.fasta")
pro_tree <- read_tree("../qiime-export/16S/tree.nwk")

#18S

#Count Table
euk_count_table <- read_tsv(file="../qiime-export/18S/table.tsv", skip=1)
...
#Rest of the data
euk_tax <- read_tsv(file="../qiime-export/18S/taxonomy.tsv")
euk_fasta <- read.fasta(file="../qiime-export/18S/dna-sequences.fasta")
euk_tree <- read_tree("../qiime-export/18S/tree.nwk")
...
#8. Remove contaminants from data objects
...
pro_TAX = tax_table(as.matrix(pro_taxonomy))
...
euk_TAX = tax_table(as.matrix(euk_taxonomy))
...
euk_ps.ambig <- subset_taxa(euk_ps, !is.na(Kingdom) & !Kingdom %in% c("Unassigned", "Bacteria", "Archaea") & !Phylum %in% c("Vertebrata", "Tardigrada", "Rotifera", "Nematozoa") & !Class %in% c("Embryophyta") & !Family %in% c("Mitochondria"))
